...
class lastReferrers extends statcommMustache
{
    ...
    public function rows()
    {
        ...
        {
            $row= array();
            $row['url'] =urldecode( utilities::irigetblogurl() .
                ((strpos($rk->urlrequested, 'index.php') === FALSE) ? $rk->urlrequested : ''));
            $row['url_ellipsis'] = urldecode(utilities::makeEllipsis($row['url'], 70));
            $row['date'] = utilities::conv2Date($rk->date);
            $row['time'] = $rk->time;
            $row['referrer'] = $rk->referrer;
            $row['referrer_ellipsis'] = urldecode(utilities::makeEllipsis($rk->referrer, 70));
            $results[]=$row;
        }
        return $results;
    }
