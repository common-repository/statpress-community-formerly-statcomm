...
class lastHits extends statcommMustache
{
    ...
    public function title() {return __("Last Hits","statcomm");}
    ...
    {
        $this->displayGIPC=utilities::geolocationEnabled();
        $this->displayUASC=utilities::uaParserEnabled();
    }

    //This option excludes columns Flag, Region and Country if Maxmind is disabled or incorrect
    public function displayGeoIPColumns()
    {
        return ($this->displayGIPC == utilities::ERROR_NONE);
    }

    //This option excludes Operative System, Browser Engine and Browser Version if UAS is disabled or incorrect
    public function displayUAScolumns()
    {
        return ($this->displayUASC == utilities::ERROR_NONE);
    }

    public function rows()
    {
        //$geoLocation=utilities::geolocationEnabled();
        $geoLocation=$this->displayGeoIPColumns();
        $gi=NULL;
        $parser= new statcommParser();
        $lastHits = mySql::get_results(mySql::QRY_lastHits,200);

        if ($geoLocation )  { $gi = utilities::geoLocationOpen(); }
        ...
        foreach ($lastHits as $hit) {
            ...
            $row['url'] = urldecode(utilities::outUrlDecode($hit->urlrequested));
            $row['url_requested'] = urldecode($hit->urlrequested);
            $row['url_ellipsis'] = urldecode(utilities::makeEllipsis($row['url'], 40));
        ...
        if($geoLocation){ utilities::geoLocationClose($gi); }
