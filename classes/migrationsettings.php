...
class settingsMigrationAPI
{
    ...
    static function statcomm_migration_option_page() {
        ...
        if ($enabled)
        {
            ...
            utilities::fl ("isTimeLimitEnabled",$istle[0]);
            utilities::fl ("isSafeModeEnabled",self::isSafeModeEnabled());
        }
        ?>
    <div class="wrap">
     ...
     * 1.7.50: Improved safe_ini evaluation.
     * @static
     * @return bool
     */
    static function isSafeModeEnabled()
    {
        $isSME=ini_get('safe_mode');
        if (empty($isSME)){
            return false;
        }
        switch (strtolower($isSME))
        {
            case 'on':
            case 'yes':
            case 'true':
