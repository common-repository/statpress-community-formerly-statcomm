...
class mySql implements dbLayer
{
    ...
    //Test datatable
    Const QRY_lastHits_dt      = "SELECT * FROM #table# WHERE (os<>'' OR feed<>'') and date='#param#' order by id DESC limit #param#";
     ...
     * @return mixed
     */
    private static function qryReplace($qry,$parameters)
    {
  ...
  KEY index_feed (feed) USING BTREE
    ...
    */
     ...
     */
    static public function statcommJson($aColumns,$sIndexColumn, $sTable)
    {
    /* Paging  */
    $sLimit = "";
    $iDisplayStart  = isset($_GET['iDisplayStart'])?$_GET['iDisplayStart']:"";
    $iDisplayLength = isset($_GET['iDisplayLength'])?$_GET['iDisplayLength']:"";
    $iSortCol_0     = isset($_GET['iSortCol_0'])?$_GET['iSortCol_0']:"";
    $iSortingCols   = isset($_GET['iSortingCols'])?$_GET['iSortingCols']:"";
    ...
    $rResult = self::get_results($sQuery,null,ARRAY_A);
    //$rResult = mysql_query( $sQuery, $gaSql['link'] ) or fatal_error( 'MySQL Error: ' . mysql_errno() );
    //TODO:Control error here.
    /* Data set length after filtering */
    //$rResultFilterTotal = mysql_query( $sQuery, $gaSql['link'] ) or fatal_error( 'MySQL Error: ' . mysql_errno() );
     // $rResultFilterTotal = self::get_results($sQuery);
    //$aResultFilterTotal = mysql_fetch_array($rResultFilterTotal);
//        $iFilteredTotal = $aResultFilterTotal[0];

    /* Total data set length */
    $sQuery = " SELECT COUNT(`" . $sIndexColumn. "`)  FROM   $sTable  ";
    $iTotal = self::get_row($sQuery,null,ARRAY_N);
    $iTotal = $iTotal[0];
     utilities::fl("Get ROW Totals Previous:", $iTotal);

    /* Output */
     utilities::fl("Query:", $sQuery);
     utilities::fl("Get ROW Totals:", $iTotal);
