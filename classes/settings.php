...
class settingsAPI
{
    ...
    /**
     * 1.7.60: Improved security
     * Rules: only administrator or network administrator should be able to change settings
     * Users below administrator can only read the settings
    ...
    <div class="wrap">
        ...
        {
            //I couldn't find a way to pass DISABLED to submit_button.
            ?>

            <p class="submit">
                <input id="submit" class="button on" type="submit"
                       value="<?php _e('Only Admins can Save Settings','statcomm');?>" DISABLED name="submit">
            </p>
    ...
    function statcomm_advanced_options()
    {
        ...
        if (!is_multisite())  //At the same time, a warning is issued to the user(s)
        {
            ...
            add_settings_field('statcomm_chk_uas',
                __('Check for disabling User Agent String Database','statcomm'),
                array( &$this, 'statcomm_setting_chk_uas'),
                self::SC_ADVANCED_OPTIONS_KEY, 'statcomm_advanced_options');

            add_settings_field('statcomm_chk_disable_maxmind',
                __('Check for disabling Maxmind','statcomm'),
                array( &$this, 'statcomm_setting_chk_maxmind'),
                self::SC_ADVANCED_OPTIONS_KEY,  'statcomm_advanced_options');

            //20130128:Removed comments, since check is above the link.
         //   add_settings_field('statcomm_chk_gcl',   __('','statcomm'),array( &$this, 'statcomm_setting_chk_glc'),   self::SC_ADVANCED_OPTIONS_KEY,  'statcomm_advanced_options');
    ...
    //v1.7.50: added present of mb_internal_encoding. This could be a problem to make GeoIP works.
    //v1.7.60: improved file detection for UAS database.
    //v1.7.68: suppressed mb_internal_encoding checking. This is the first thing plugin checks before install

    function statcomm_section_text() {
        //echo '<p>' . __('Current settings for StatComm','statcomm') . '</p>';
        $green = "<span style='color:GREEN;'> OK </span>";
        $red   = "<span style='color:RED;'> ERROR </span>";
        ...
        print "<td><b>" . $flagZip . "<b/></td></tr>";

        print "<tr><td>" . __('Checking if folder def is writable (used for save UAS database)','statcomm') . "</td>";
        print "<td><b>" . $flagCache . "<b/></td></tr>";


        print "<tr><td>" . __('Current User Agent String DB version','statcomm') . "</td>";

        switch(utilities::uaParserEnabled())
        {
            case utilities::ERROR_DISABLED:
                print "<td><b style='color:RED;'>" .  __("User Agent String database disabled. Activate it on Advanced Tab","statcomm") . "</b></td></tr>";
                break;

            case utilities::ERROR_FILE_NOT_FOUND:
                print "<td><b>" . $red . __("Cache.ini or Uasdata.ini not found or not readable. Try installing UAS in Advanced Options","statcomm") . "</b></td></tr>";
                break;

            case utilities::ERROR_NONE:
                //print "<td><b>" . $localversion . "</b> dowloaded in date " . gmdate("Y-m-d h:i:s",$lastupdate) .   "</td></tr>";
                print "<td>" .__(sprintf("<b>%s </b> dowloaded in date %s", $localversion, gmdate("Y-m-d h:i:s",$lastupdate)),"statcomm")  .   "</td></tr>";
                break;
            default:
                print "<td><b>" .  __("Un classified error. Call de programmer!","statcomm") . "</b></td></tr>";
        }

        /* Suppressed check
        print "<tr><td>" . __('Checking if multibyte string extension is installed (mb_internal_encoding)','statcomm') . "</td>";
        if (function_exists("mb_internal_encoding"))
        {
            print "<td><b>" . $green . __("Is safe to use Maxmind database","statcomm") . "</b></td></tr>";
        }
        else
        {
            print "<td><b>" . $red . __("Please install Multibyte String Extension Module on Apache Server","statcomm") . "<br/>" .
                             __("Check this link to see what is all about:","statcomm") .
                              " <a href='http://forum.wpgetready.com/index.php?topic=222.0' target='_blank'>Fatal error when Multibyte String Extension is not installed</a></b><br/>" .
                              __("It is NOT safe using Maxmind database in the current Apache configuration! (you will have errors)","statcomm") . "</td></tr>";
        }
        */
        print "</tr>\n";
        print "</table></div>";
     ...
     * 1.7.60 - Improved since we can disable Maxmind manually
     * @static
     *
     */
    function statcomm_setting_chk_glc() {
        ...
        switch ($geoLocation) {
            case utilities::ERROR_NONE:
                ...
                echo    __('  Maxmind database installed and ready. ','statcomm');
                $this->maxmindDownloadLink();
                echo "</span>";
                break;
            ...
            case utilities::ERROR_DISABLED:
                echo "<span style='color:RED;'>" . __('Maxmind database disabled. Columns [Flag], [Region] and [City] will be excluded from reports.', 'statcomm') .'</span><br/>' ;
                break;
            ...
            case utilities::ERROR_INCORRECT_VERSION:
                echo "<span style='color:RED;'>" . __(' Unable to open Maxmind database(incorrect version?).', 'statcomm');
                $this->maxmindDownloadLink();
                echo "</span>";
                break;

            default:
                echo "<span style='color:RED;'>" . __(' Unexpected error from geoLocationEnabled. Contact with the programmer!!!', 'statcomm') ;
                break;
        }
    ...
    /**
     * Makes Maxmind link according user's permissions
     */
    function maxmindDownloadLink()
    {
        if (current_user_can('activate_plugins'))
        {
            echo '<a href="' . wp_nonce_url(admin_url( 'admin.php?page=statComm/glcupdate' ),'statcomm'). '" >'.
                __('Click to start downloading', 'statcomm'). '</a>';
        }
        else
        {
            echo " ";
            _e('(Download only available for Administrators.)', 'statcomm');
        }
    }
     ...
     * v1.7.60 Optionally disable UAS in case user has problems.
     * @static
     *
     */
    function statcomm_setting_chk_uas() {
        ...
        $chk =isset($options['chk_disable_uas'])?$options['chk_disable_uas']:"";
        $checked='';
        if($chk) { $checked = ' checked="checked" '; }
        echo "<input id='chk_disable_uas' name='" . self::SC_ADVANCED_OPTIONS_KEY. "[chk_disable_uas]' type='checkbox' $checked/> ";
        //Display validation, this will work both for single and multisite
        self::statcomm_setting_chk_uas_validation();
        echo "<p/>";
    }

    /**
     * 1.7.60: Improved validation only for admin users.
     */
    function statcomm_setting_chk_uas_validation()
    {
        switch (utilities::uaParserEnabled()){
            case utilities::ERROR_DISABLED:
                echo "<span style='color:RED;'>" .__(' UAS database disabled. Columns [Operative System], [Browser Engine] and [Browser Version] will be excluded from reports. ','statcomm') . "</span>" ;
                break;
            case utilities::ERROR_FILE_NOT_FOUND:
                echo "<span style='color:RED;'>" . __('  You need to install User Agent String database.', 'statcomm') ;
                $this->uaparserDownloadLink();
                echo "</span>";
                break;
            case utilities::ERROR_NONE:
                ...
                if (!$isUpdated['status'])
                {
                    ...
                    if (!empty($msg))
                    {
                        ...
                        echo "<span style='color:RED;'>" . sprintf(__("There was an error when checking if updated: %s", "statcomm"),$msg) . "</span>";
                    }
                    else
                    {
                        echo "<span style='color:GREEN;'>";
                        echo __('  There is a new UAS version to install. ','statcomm');
                        $this->uaparserDownloadLink();
                        echo "</span>";
                    }
                }
                ...
                {
                    echo "<span style='color:GREEN;'>" .__('  UAS database installed and ready. ','statcomm') . "</span>" ;
                }
                break;
            default:
                echo "<span style='color:RED;'>" . __("Unknown error. Contact with the programmer!!!", "statcomm") . "</span>";
        }
    }

    /**
     * Makes the download link according the user's permissions.
     */
    function uaparserDownloadLink()
    {
      if (current_user_can('activate_plugins'))
      {
          echo '<a href="' . wp_nonce_url(admin_url( 'admin.php?page=statComm/uasupdate' ),'statcomm'). '" >' .
              __('Click here to install', 'statcomm'). '</a>';
      }
        else
        {
            echo " ";
    ...
    /**
     * v1.7.60: manually enable/disable maxmind database if you have problems updating this database
     */
    function statcomm_setting_chk_maxmind() {
     ...
     //   self::statcomm_setting_chk_glc;
        settingsAPI::statcomm_setting_chk_glc();
    }

    /**
     * v1.7.20: moved delete table to advance tab.
     ...
     * @param $input
     * @return array
     * 1.7.60, update using chk_disable_maxmind
     */
    function sc_validate_advanced_options($input)
    {
        ...
        $valid['chk_disable_maxmind'] = isset($input['chk_disable_maxmind'])?$input['chk_disable_maxmind']:"";
        $valid['chk_disable_uas'] = isset($input['chk_disable_uas'])?$input['chk_disable_uas']:"";
        return $valid;
    }
    ...
    static function defaultValues()
    {
        ...
        $options_advanced=  array(
            'chk_deltable' => FALSE,
            'chk_disable_maxmind' =>FALSE,
            'chk_disable_uas' =>FALSE
