...
/*
   ...
   StatPress Community is a plugin for web statistics, based in their StatPress sucessors.

    v1.7.68:
    Plugin checks before setup if can be installed.
    Current limitations: WP>3.3 and mbstring extension installed (mb_internal_encoding has to exists)
*/
  ...
  class statPressCommunity
  {
    ...
    const BANIP_FILE = 'banips.dat';    //List of IP banned from the plugin
   ...
   {
        register_activation_hook(__FILE__,  'utilities::canInstall'); //checks if plugin can be installed
       ...
       //Main command for ajax control.
       add_action('wp_ajax_StatcommAjax', 'viewSystem::StatcommAjax' );//See templates.php!
  ...
  function addPages()
  {
      ...
      $this->currentPage=$statcomm_style;
...
//1.7.00
 //     add_submenu_page('statComm', __('Migration','statcomm'), __('Migration','statcomm'), $mincap, 'statComm/migration', array(&$this, 'statcommMigrationOptions'));
      add_submenu_page('statComm', __('Migration','statcomm'), __('Migration','statcomm'), $mincap, 'statComm/migration', 'settingsMigrationAPI::statcomm_migration_option_page');
      ...
      //1.7.60: Added style for controlling error output
      $uasupdate=add_submenu_page('statComm/options', __('UAS parser update'       ,'statcomm'), __('Options'       ,'statcomm'), $mincap, 'statComm/uasupdate', array(&$this, 'statcommUASUpdate'));
      add_action( 'admin_print_styles-' . $uasupdate  , 'statPressCommunity::statcommUASUpdateStyle' );
      ...
      static function jsScriptsPage()
      {
          ...
          //Note: strictly speaking, we don't pass null, we should pass array(null). Don't it: the script won't work.
          ...
          //Javascript detection and initialization
          ...
          //v1.7.50: adding datatables
          //wp_enqueue_script('datatables', plugin_dir_url(__FILE__) . 'js/jquery.dataTables.min.js', array('jquery'), false, true);
          wp_enqueue_script('datatables', plugin_dir_url(__FILE__) . 'js/jquery.dataTables.js', array('jquery'), FALSE, TRUE);

          //1.7.60: added javascript for metaboxes
   ...
   * 1.7.50: added table style
   */
  function addDetailsStyle()
  {
      wp_register_style( 'statcomm-sprites',    plugins_url('css/statcomm-sprites.css', __FILE__), array());
      ...
      wp_register_style( 'statcomm-table',      plugins_url('css/statcomm-table.css', __FILE__), array() );
  }

      /**
       * 1.6.5: Register custom style for proper flag into Details page.
       * 1.7.50: converted to static
       * 1.7.60: separating sprites to test new subplugin
       */
  static function statcommAdminStyle()
  {
      ...
      wp_enqueue_style('statcomm-table');
  }

  /**
   * 1.7.60: added style to UASUpdate to control error output.
   * wp_register_style always done by main process
   */
  static function statcommUASUpdateStyle()
  {
      wp_enqueue_style('statcomm-admin-style');
  }
 ...
 * Update: statcommMain organized in much more manageable modules and improved documentation.
 ...
 */
      ...
      {
          $v=new viewSystem($this->viewPath, "statcomm",$this->currentPage);
          $v->render();
          return;
      }
              ...
              if ($dst == 'checked')
{
   $fields = " DISTINCT " . $fields;
       ...
       * @param $IpValue
       * @return bool
       * 1.7.70: Rewritten, improved and simplfied.
       */
	  function checkBanIp($IpValue)
      {
        global $wpdb;
        $banIpDir= self::getBanIpDir();             //get where banips.dat is
        if ($banIpDir === FALSE) return FALSE;      //if banips.dat is missing, ignore this check
        return self::checkIPOverFile($IpValue,$banIpDir);
      }

      /**
       * get BanIp dir. It can be overriden using a custom banip file on a [plugin_folder_name]-custom folder with
       * the banips.dat file.
       * This method has Unit Tests
       * Returns dir for banips.dat file
       * TODO: Is this documented on help files?
       * @return bool|string
       */
      function getBanIpDir($filename=self::BANIP_FILE) {
          $basePath         =   WP_CONTENT_DIR . '/plugins/' . dirname(plugin_basename(__FILE__));
          $pathCustomBan    =   $basePath . "-custom/$filename" ; //Check if user created a custom .dat file
          $pathBan          =   $basePath . "/def/$filename" ;    //default .dat file

          if (file_exists($pathCustomBan)){ //If custom Banip file exists, it overrides default
              return $pathCustomBan;
          }
          if (file_exists($pathBan)){       //Custom Banip list is missing. Check if default banip file exists.
              return $pathBan;
          }
          return FALSE;                     //No banips.dat file present
      }

      /**
       * Verifies if IP is present on a file.
       * This method has Unit Tests
       * @param $IP
       * @param $Filename
       */
      function checkIPOverFile($IP,$Filename) {
          global $wpdb;
          if (!file_exists($Filename)) return FALSE;   //This case should be throw an exception instead returning FALSE
          $lines = file($Filename);                   //banips.dat is present, check if we have a match
          if ($lines === FALSE) return FALSE;         //If there were problems loading the file, skip the process.
          foreach ($lines as $BanIp){
              if (@preg_match('/^' . rtrim($BanIp, "\r\n") . '$/', $IP)){
                  utilities::fl("Match $BanIp <-> $IP in blog Id", $wpdb->blogid);
                  return TRUE;
              }
          }
          return FALSE;
...
/**
 * Decides if it has to upgrade the plugin and als if it can install it
 ...
 */
       ...
       * v1.7.60: user information is saved even is not recorded in database. That could be the case for logged users with track disabled
       *threat analysis not currently implemented since StatComm 1.6.3. In analysis
       * @return string
       * TODO: there should be a 'reason' for not appending data.(error code or msg)
       */
	  public function statcommAppend()
      {
        //TODO: buscar la manera de modularizar y simplificar este metodo lo más posible.
        global $userdata;
        ...
        //IMPROVE:
        //http://wordpress.org/support/topic/plugin-statcomm-statpress-community-multisite-edition-correct-ip-address-behind-reverse-proxy
        //Thanks to: rogst . Correction when wp is behind a reverse proxy.

        $ipAddress = isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
        if ($this->checkBanIp($ipAddress) === TRUE)
        {
		...
		$extension = pathinfo( $urlRequested, PATHINFO_EXTENSION ); //Extract extension (if any)
		$commonFileTypes='/ico|css|js|jpe?g|png|gif/'; 		        //Filter by extension, discard common filetypes
		...
		//1.6.4: modified specialfolder delimiter
        ...
        else //if not a Spider, is a feed?
        {
            ...
            $os = $agent['os_name'];                                // Get OS and browser
            $browser = $agent['ua_family'];                         //it can be ua_version but it has far more details
            $pathDefinitionsFile=plugin_dir_path(__FILE__) . 'def'; //get search_phrase using searchterms
        ...
        //v1.7.00: Introducing filter mode. Notice that:
        ...
        $ud=isset($userdata->user_login)?$userdata->user_login:"";
          ...
          //If the user returns null or empty string ("") the append is avoided.
          //In this case subplugin shortcode will return nothing!!!
          if(empty($scu)) return '';
          ...
          $data   = self::moveToArray($scu); //Transfer data object to array for proper database insertion
      ...
      if (class_exists("statcommCurrentUser"))
      {
          ...
          //Save results in statcomm_options.
          $options = get_option('statcomm_options'); //get all options from the plugin
          //Convert object so it is easier to store
          //Fix 'Object of class statcommCurrentUser could not be converted to string' on some PHP Versions & configuration
          $options['currentUser'] = base64_encode(serialize($scu));
          update_option('statcomm_options', $options);
      }
      utilities::fl ("Time completed t1:", utilities::stopTimer($t1));
      return '';
      }
      /*917-1100= 183 line code*/
      /*20130613 = 918 -1091 = 1073 code lines*/

      /**
       * Mover el objeto a un array para insertarlo a la base de datos
       * @param statcommCurrentUser $scu
       * @return array
       */
      private function moveToArray (statcommCurrentUser $scu){
          $data=array();
          $data[]=$scu->getDate();
          $data[]=$scu->getTime();
          $data[]=$scu->getIp();
